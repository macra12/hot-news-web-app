...
function CopyLinkButton() {
  ...
  return (
    <button
      onClick={copy}
      className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-800 hover:bg-gray-700 border border-gray-700 text-gray-400 hover:text-white text-xs font-semibold transition-all"
    >
      {copied ? (
        <>
...
function RecommendedCard({ article }) {
  return (
    <Link href={`/news/${article.slug}`} className="group flex gap-4 p-4 rounded-xl bg-gray-900/60 border border-gray-800 hover:border-red-600/40 hover:bg-gray-900 transition-all duration-200">
      {article.image && (
      ...
      <div className="min-w-0 flex flex-col gap-1">
        ...
        <h4 className="text-white text-sm font-semibold leading-snug line-clamp-2 group-hover:text-red-400 transition-colors">
          {article.title}
        </h4>
        <span className="text-gray-600 text-[11px] mt-auto">
...
export default function ArticleDetail() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      fetch(`${API_BASE}/news/${data.id}/increment_view/`, { method: "POST" }).catch(() => {});
  ...
  if (notFound || !article) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-4 bg-gray-950">
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-12">
        ...
        <div>
          ...
          <Link
            href="/news"
            className="inline-flex items-center gap-1.5 text-gray-500 hover:text-red-400 text-sm transition-colors group mb-8"
          >
          ...
          <article>
            ...
            {/* Summary / lead */}
            {article.summary && (
              <p className="text-lg text-gray-300 leading-relaxed mb-6 pl-4 border-l-[3px] border-red-600">
                {article.summary}
              </p>
            )}

            {/* Meta row */}
            <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-8 pb-6 border-b border-gray-800">
            ...
            <div className="space-y-4">
              {article.content?.split("\n").map((para, i) =>
                para.trim() ? (
                  <p key={i} className="text-gray-300 leading-[1.85] text-[15px]">
                    {para}
                  </p>
                ) : null
              )}
            ...
            <div className="mt-10 pt-8 border-t border-gray-800 flex flex-wrap items-center gap-3">
              <span className="text-gray-600 text-xs font-semibold uppercase tracking-wider mr-1">Share</span>
              <CopyLinkButton />
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent(typeof window !== "undefined" ? window.location.href : "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gray-800 hover:bg-[#1d9bf0]/20 hover:border-[#1d9bf0]/50 border border-gray-700 text-gray-400 hover:text-[#1d9bf0] text-xs font-semibold transition-all"
              >
              ...
              {categorySlug && (
                <Link
                  ...
                  className="ml-auto inline-flex items-center gap-1.5 text-xs text-gray-600 hover:text-red-400 transition-colors"
                >
          ...
          {recommended.length > 0 && (
            <div className="mt-14 lg:hidden">
              <div className="flex items-center gap-2 mb-5">
                ...
                <h2 className="text-lg font-bold text-white">More in {categoryName || "News"}</h2>
              </div>
        ...
        <aside className="hidden lg:block space-y-6 pt-11">
          ...
          <div className="sticky top-24">
            ...
            {recLoading ? (
              ...
              <p className="text-gray-600 text-sm">No related articles yet.</p>
            )}
            ...
            {categorySlug && (
              <Link
                href={`/category/${categorySlug}`}
                className="mt-4 flex items-center justify-center gap-1.5 w-full py-2.5 rounded-xl border border-gray-800 hover:border-red-600/50 text-gray-500 hover:text-red-400 text-xs font-semibold transition-all"
              >
            ...
            <div className="mt-6 pt-6 border-t border-gray-800 flex flex-col gap-2">
              <Link
                href="/news"
                className="flex items-center gap-2 text-gray-600 hover:text-gray-300 text-xs transition-colors"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                </svg>
